...
Utilize o gráfico a seguir:  
**Anexo:** [Curva da bomba e curva do sistema](sandbox:/mnt/data/grafico_bomba_sistema.png)

Com base no gráfico, assinale a alternativa correta:

A) O ponto de operação é definido exclusivamente pela curva da bomba.  
B) O ponto de operação desloca-se para maior vazão quando a curva do sistema se torna mais inclinada.  
C) O ponto de operação corresponde à interseção entre a curva da bomba e a curva do sistema.  
D) A vazão no ponto de operação independe das perdas por atrito.  
E) Em qualquer sistema, a bomba sempre opera no ponto de maior eficiência hidráulica.

---

## Questão 3 — Resposta única

**Tema:** NPSH e cavitação  
**Suporte:** mini-enunciado numérico

Em uma instalação de bombeamento de água a $25^\circ\text{C}$, o reservatório de sucção está aberto à atmosfera. Considere:

- pressão atmosférica expressa como coluna d’água: $10{,}3\ \text{m}$
    
- altura estática de sucção: $2{,}0\ \text{m}$
    
- perdas de carga na sucção: $1{,}2\ \text{m}$
    
- pressão de vapor da água na temperatura de operação: $0{,}3\ \text{m}$ de coluna d’água
    

Desprezando a parcela cinética, calcule o **NPSH disponível** e informe se a bomba operará sem cavitação caso o fabricante especifique $NPSH_{req}=8{,}0\ \text{m}$.

**Resposta esperada:** valor numérico em metros e conclusão técnica.

**Texto de apoio para a prova:**  
O $NPSH$ disponível representa o saldo positivo de energia na sucção necessário para evitar vaporização local e colapso de bolhas no interior da bomba; o $NPSH_{req}$ é fornecido pelo fabricante.
